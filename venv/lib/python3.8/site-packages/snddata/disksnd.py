...
__all__ = ['asdisksnd', 'create_disksnd', 'delete_disksnd', 'DiskSnd']

# scalingfactor
# append?
class DiskSnd(BaseSnd):
    ...
    def set_startdatetime(self, startdatetime):
        self._diskarray.update_metadata(startdatetime=startdatetime)


    def copy(self, path, dtype=None, accessmode='r', chunksize=44100, overwrite=False):
        return self._diskarray.copy(path=path, dtype=dtype, chunksize=chunksize,
...
def asdisksnd(path, array, fs, scalingfactor=None, startdatetime='NaT',
    ...
    da.update_metadata(sndmetadata)
    return DiskSnd(path=path, accessmode=accessmode)
...
def create_disksnd(path, nframes, nchannels, fs, startdatetime='NaT',
    ...
    da = create_array(path=path, shape=shape,
                   ...
                   chunksize=chunksize, metadata=metadata, overwrite=overwrite)
