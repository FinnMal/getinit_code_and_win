...
def audiofiles_to_disksnd(audiofilepaths, sndpath, dtype='float32',
    ...
    for afp in audiofilepaths[1:]:
        af = AudioFile(filepath=afp, dtype=dtype, mode='r')
        assert af.nchannels == snd.nchannels
        assert af.fs == snd.fs
        snd._diskarray.append(af.iterread_frames(framesize=framesize))
    return snd
...
def audiofile_to_disksnd(audiofilepath, sndpath, dtype='float32',
    ...
    gen = af.iterread_frames(framesize=framesize, stepsize=None,
                             include_remainder=True, startframe=startframe,
                             endframe=endframe)
    snd = asdisksnd(path=sndpath, array=gen, fs=af.fs, dtype=dtype,
                    startdatetime=startdatetime, metadata=metadata, mode=mode)
    return snd
